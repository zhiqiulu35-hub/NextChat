...
// OKX 交易 API 凭证（通过环境变量设置，不得硬编码在代码中）
...
// TradFi 股票永续合约 = 无连字符、以USDT结尾（如 NVDAUSDT）
// 代币化股票现货 = 以X开头（如 XNVDA, XTSLA）
function detectInstType(instId: string): "SPOT" | "SWAP" | "FUTURES" {
  const id = instId.toUpperCase().trim();
  if (id.includes("-SWAP")) return "SWAP";
  if (id.includes("-") && /\d{6,8}$/.test(id.split("-").pop() || ""))
    return "FUTURES";
  if (!id.includes("-") && id.endsWith("USDT")) return "SWAP"; // TradFi 股票永续合约
  return "SPOT"; // 默认 SPOT
}

function isTradFi(instId: string): boolean {
  return detectInstType(instId) === "SWAP" && !instId.includes("-SWAP");
}
...
async function exec(tool: string, p: Record<string, any>) {
  switch (tool) {
    ...
    case "candles": {
      ...
      return fmtCandles(
        await okxPub(
          `/market/candles?instId=${e(p.instId)}&bar=${
            p.bar || "1H"
          }&limit=${Math.min(p.limit || 20, 300)}`,
        ),
        p.instId,
        p.bar || "1H",
      );
    ...
    case "orderbook": {
      ...
      return fmtBook(
        await okxPub(
          `/market/books?instId=${e(p.instId)}&sz=${Math.min(
            p.size || 20,
            400,
          )}`,
        ),
    ...
    case "funding": {
      req(p, ["instId"]);
      return fmtFunding(
        await okxPub(`/public/funding-rate?instId=${e(p.instId)}`),
      );
    }
    ...
    case "overview": {
      ...
      const [tk, cl, ob] = await Promise.all([
        ...
        okxPub(`/market/candles?instId=${e(id)}&bar=1D&limit=7`).catch(
          () => null,
        ),
        okxPub(`/market/books?instId=${e(id)}&sz=10`).catch(() => null),
      ]);
      return {
        summary: fmtTicker(tk),
        trend: cl ? fmtCandles(cl, id, "1D") : null,
        depth: ob ? fmtBook(ob) : null,
      };
    }
    // ---- TradFi 股票永续合约 ----
    case "get_tradfi_ticker":
    case "tradfi": {
      req(p, ["instId"]);
      const instId = p.instId.toUpperCase();
      // 自动补全 USDT 后缀
      const fullInstId = instId.endsWith("USDT") ? instId : instId + "USDT";
      const data = fmtTicker(
        await okxPub(`/market/ticker?instId=${e(fullInstId)}`),
      );
      if (data) data.type = "tradfi_stock";
      return data;
    }
    case "get_tradfi_stocks":
    case "tradfi_list": {
      const results = [];
      for (let i = 0; i < TRADFI_STOCKS.length; i += 5) {
        const batch = TRADFI_STOCKS.slice(i, i + 5);
        const br = await Promise.all(
          batch.map((s) =>
            okxPub(`/market/ticker?instId=${e(s)}`)
              .then(fmtTicker)
              .catch(() => null),
          ),
        );
      ...
      return {
        description: "OKX 股票永续合约实时行情（TradFi 板块）",
        count: results.length,
        stocks: results,
      };
    }

    case "get_hot_cryptos":
    case "hot": {
      const r = [];
    ...
    case "get_major_indices":
    case "indices": {
      const syms = INDICES.map((x) => x.s).join(",");
      const q = await yq(syms);
      return {
        indices: INDICES.map((x, i) => ({
          name: x.n,
          symbol: x.s,
          quote: q.results?.[i] || null,
        })),
      };
    }
    ...
    case "trade_get_order":
    case "get_order": {
      req(p, ["instId", "ordId"]);
      return await getOrder(p.instId, p.ordId);
    }
    case "trade_get_balance":
    case "balance":
    case "get_balance": {
...
async function placeLimitOrder(p: Record<string, any>) {
  ...
  if (orderValue > MAX_ORDER_USDT) {
    return {
      warning: true,
      message: `⚠️ 订单金额 $${orderValue.toFixed(
        2,
      )} 超过单笔上限 $${MAX_ORDER_USDT}`,
  ...
  if (p.confirmed !== true && p.confirmed !== "true") {
    return {
      ...
      message: `请确认：${side === "buy" ? "买入" : "卖出"} ${p.sz} ${
        p.instId
      }，限价 $${p.px}，预估 $${orderValue.toFixed(2)}`,
      hint: "确认请将 confirmed 设为 true，如: { ... params: { ..., confirmed: true } }",
    };
  }

  // 检测产品类型，动态设置交易参数
  const instType = detectInstType(p.instId);
  const instId = p.instId.toUpperCase();

  const orderBody: Record<string, any> = {
    instId,
    tdMode: instType === "SPOT" ? "cash" : "isolated",
    side,
    ordType: "limit",
    px: String(p.px),
    sz: String(p.sz),
  ...
  return {
    ...
    amount: p.sz,
    state: "已提交",
    message: `✅ 已提交${side === "buy" ? "买入" : "卖出"}委托：${p.sz} ${
      p.instId
    }，限价 $${p.px}`,
  };
}
...
async function getOrder(instId: string, ordId: string) {
  const result = await okxAuth(
    "GET",
    `/api/v5/trade/order?instId=${e(instId.toUpperCase())}&ordId=${e(ordId)}`,
  );
...
async function getBalance() {
  ...
  return {
    ...
    summary: details
      .map(
        (d: any) => `${d.currency}: ${d.balance} (≈ $${d.usdt_value || "0"})`,
      )
      .join(" | "),
  };
}
...
function fmtTicker(d: any) {
  ...
  const last = parseFloat(t.last || "0"),
    open = parseFloat(t.open24h || t.last || "1");
  return {
    type: "crypto",
    pair: t.instId,
    price: t.last,
...
function fmtFunding(d: any) {
  ...
  return {
    pair: f.instId,
    rate: f.fundingRate,
    nextTime: f.fundingTime,
    est: f.estimatedRate,
  };
}

function fyq(item: any) {
  const ch = item.regularMarketChange,
    cp = item.regularMarketChangePercent;
  return {
    type: "stock",
    symbol: item.symbol,
    ...
    change: ch,
    changePct: cp,
    changePctStr: cp != null ? `${cp >= 0 ? "+" : ""}${cp.toFixed(2)}%` : null,
    marketState: item.marketState,
    afterHours: item.postMarketPrice,
    preMarket: item.preMarketPrice,
    marketCap: item.marketCap,
    marketCapStr: item.marketCap ? fmc(item.marketCap) : null,
    pe: item.trailingPE || item.forwardPE,
    divYield: item.dividendYield
      ? `${(item.dividendYield * 100).toFixed(2)}%`
      : null,
    wkHigh: item.fiftyTwoWeekHigh,
    wkLow: item.fiftyTwoWeekLow,
    vol: item.regularMarketVolume,
    avgVol: item.averageDailyVolume3Month,
    ts: item.regularMarketTime,
  };
}
